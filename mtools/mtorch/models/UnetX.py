...
class Unet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, channels=[32, 64, 128, 256]):
        ...
        self.downup = nn.ModuleList()
        for channel in channels:
            self.downup.append(nn.Conv2d(in_channels=channel, out_channels=out_channels, kernel_size=(1, 1), stride=1))
    ...
    def forward(self, image):
        ...
        downs = []
        for module, tensor in zip(self.downup, skipes[1:]):
            downs.append(torch.sigmoid(module(F.interpolate(tensor, size=skipes[0].size()[2:]))))
